            ...
            <div className="project__content">
                ...
                <ul  className="project__github">
                {
                    this.props.data.github.map((linkData, index) =>
                        ( <li key={index} className="project__github--item"> <a className="btn btn--white" href={linkData.link} target="_blank" > {linkData.title} </a> </li>)
                    )
                }
                </ul>
                <h3 className="heading-tertiary u-margin-bot-sm"> Deployed Site </h3>
                <a className="btn btn--white" href={this.props.data.deployed.link} target="_blank" > {this.props.data.deployed.title} </a>
            </div>
        </div>
        )
    }
